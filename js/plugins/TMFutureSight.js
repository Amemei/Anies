...
TMPlugin.FutureSight.MaxLines       = +(TMPlugin.FutureSight.Parameters['maxLines'] || 2);
...
(function() {
  ...
  Game_Enemy.prototype.setFutureSightText = function(text) {
    this._futureSightText = text;
  };

  Game_Enemy.prototype.futureSightText = function() {
    return this._futureSightText;
  };
  ...
  Game_Troop.prototype.makeActions = function() {
    ...
    if ($gameSystem.isFutureSightEnabled() && !BattleManager._preemptive) {
      this.members().forEach(function(member) {
        var action = member.currentAction();
        if (action) {
          var skill = action.item();
          if (skill) member.setFutureSightText(skill.meta.fsText || skill.name);
        }
      });
    }
  };
  ...
  Game_Troop.prototype.increaseTurn = function() {
    ...
    this.members().forEach(function(member) {
      member.setFutureSightText(null);
    });
  };
  ...
  Sprite_FutureSight.prototype.refresh = function() {
    ...
    if (this._text) {
      ...
      for (var i = 0; i < arr.length; i++) {
        this.bitmap.drawText(arr[i], 0, y + TMPlugin.FutureSight.LineHeight * i,
                             width, TMPlugin.FutureSight.LineHeight, 'center');
      }
    }
  };
  ...
  Sprite_FutureSight.prototype.update = function() {
    ...
    if (this._text !== this._enemySprite._enemy.futureSightText()) {
      this._text = this._enemySprite._enemy.futureSightText();
      this.refresh();
    }
